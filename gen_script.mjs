...
class WeatherStation {
  constructor(city, meanTemp) {
    this.city = city
    this.meanTemp = meanTemp
  }

  measurement() {
    let m = this.meanTemp + (Math.random() * 20 - 10); // Simulando uma distribuição gaussiana
...
const stream = fs.createWriteStream('input.txt', { flags: 'a' });
...
for (let i = 0; i < size; i++) {
  function writeData() {
    if (i > 0 && i % 50000000 === 0) {
      console.log(`Wrote ${i.toLocaleString()} measurements in ${Date.now() - start} ms`);
    }

    const station = stations[Math.floor(Math.random() * stations.length)];

    if (!stream.write(`${station.city}; ${station.measurement()}\n`)) {
...
stream.end(() => {
  console.log('All measurements have been written.');
});